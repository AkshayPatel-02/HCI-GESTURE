...
def get_gesture_info():
    return {
        "cursor_control": {
            "name": "Cursor Control",
            "description": "Move your hand in the air to control the cursor",
        ...
        "left_click": {
            ...
            "gesture": "Close index finger while keeping other fingers open",
            "image": "👆"
        },
        "right_click": {
            "name": "Right Click",
            "description": "Perform a right mouse click",
            "gesture": "Close middle finger while keeping other fingers open",
            "image": "✌️"
        },
        "scroll": {
            "name": "Scroll",
            "description": "Scroll up or down",
            "gesture": "Make a fist and move up/down",
            "image": "✊"
        },
        "volume_control": {
            "name": "Volume Control",
...
def start_gesture_control():
    ...
    if gesture_process is None:
        try:
            # Start gesture.py as a subprocess
            gesture_process = subprocess.Popen([sys.executable, 'gesture.py'], 
...
def main():
    ...
    with col2:
        ...
        # if st.button("▶️ Start Gesture Control", key="start_button"):
        #     st.session_state.gesture_control_running = True
        #     subprocess.Popen([sys.executable, "gesture.py"], 
        #                    creationflags=subprocess.CREATE_NEW_CONSOLE)
        #     st.success("Gesture Control System Started!")
            
        # # Add the stop button with red color
        # if st.button("⏹️ Stop Gesture Control", key="stop_button", 
        #             help="Click to stop the gesture control system"):
        #     st.session_state.gesture_control_running = False
        #     # Kill the gesture.py process
        #     os.system("taskkill /f /im python.exe")
        #     st.error("Gesture Control System Stopped!")
            
        # # Display status
        # if 'gesture_control_running' in st.session_state and st.session_state.gesture_control_running:
