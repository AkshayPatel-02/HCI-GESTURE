...
class HandGestureRecognizer:
    def __init__(self):
        ...
        # Scroll settings
        self.scroll_speed = 120  # Reduced for smoother scrolling
        ...
        self.minimize_cooldown = 1.0  # 1 second cooldown for minimize action
    ...
    def get_hand_state(self, hand_landmarks):
        ...
        try:
            ...
            # Check for WhatsApp gesture (thumb and pinky extended)
            is_whatsapp_gesture = (
                thumb in fingers_extended and
                pinky in fingers_extended and
                index not in fingers_extended and
                middle not in fingers_extended and
                ring not in fingers_extended
            )
            
            # Check for ChatGPT gesture (index and middle extended)
            is_chatgpt_gesture = (
                index in fingers_extended and
                middle in fingers_extended and
                thumb not in fingers_extended and
                ring not in fingers_extended and
            ...
            if is_whatsapp_gesture:
                return "WHATSAPP"
                
            if is_chatgpt_gesture:
                return "CHATGPT"
    ...
    def handle_gestures(self, hand_state, hand_landmarks=None):
        ...
        try:
            ...
            # WhatsApp gesture
            elif hand_state == "WHATSAPP" and current_time - self.last_app_launch_time > self.app_launch_cooldown:
                self.show_status("Opening WhatsApp Web")
                webbrowser.open(self.whatsapp_url)
                self.last_app_launch_time = current_time
                
            # ChatGPT gesture
            elif hand_state == "CHATGPT" and current_time - self.last_app_launch_time > self.app_launch_cooldown:
                self.show_status("Opening ChatGPT")
                webbrowser.open(self.chatgpt_url)
                self.last_app_launch_time = current_time
            ...
            #     if current_time - self.last_minimize_time > self.minimize_cooldown:
            #         self.show_status("Minimize Window")
            #         active_window = win32gui.GetForegroundWindow()
            #         win32gui.ShowWindow(active_window, win32con.SW_MINIMIZE)
...
def main():
    try:
        ...
        while True:
            ...
            key = cv2.waitKey(1)
            if key == 27 or cv2.getWindowProperty('Hand Gesture Recognition', cv2.WND_PROP_VISIBLE) < 1:
                break
